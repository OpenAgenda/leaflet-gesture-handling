(function (global, factory) {
	...
	var gestureHandlingOptions = {
	  text: {},
	  duration: 1700,
	  importLocale: (lang) => import(`./locales/${lang}.js`)
	};
	...
	var GestureHandling = L.Handler.extend({
	  ...
	  _parseGestureHandlingOptions: function() {
	    ...
	    var importLocale = this._map.options.gestureHandlingOptions.importLocale || gestureHandlingOptions.importLocale;
	    ...
	    options.importLocale = importLocale;
	    return options;
	  },
	  ...
	  _setGestureHandlingOptions: function() {
	    ...
	    (this._isLanguageContent(opts.text)
	      ? Promise.resolve(opts.text)
	      : this._getLanguageContent(opts.locale, opts.importLocale)
	    ).then((content) => {
	  ...
	  _getLanguageContent: function(lang, importLocaleFn) {
	    ...
	    if (typeof importLocaleFn === "function") {
	      Promise.resolve(importLocaleFn(lang))
	        .then(consume)
	        .catch(() => Promise.resolve(importLocaleFn(lang.split("-")[0])).then(consume)
	          .catch(() => Promise.resolve(defaultLocale).then(consume)));
	    }
	    else {
	      consume(defaultLocale);
	    }
